...
import { getContext } from '../utils/global'
...
export default class ReaderViewProvider implements WebviewViewProvider {
        ...
        switch (message.command) {
          ...
          case 'set_home':
            ...
            window.showInformationMessage(
              'Successfully set default online gallery page / 设置默认在线图库页面成功！',
            )
            break
        }
      },
  ...
  private getHtmlForWebview(page?: string) {
    ...
    if (page === 'home') {
      ...
      // 获取vscode本地全局变量
      let context = getContext()
      let backgroundCoverOnlineDefault: string | undefined = context.globalState.get(
        'backgroundCoverOnlineDefault',
      )
